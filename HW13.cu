...
void makeBitMap()
{	
	//cudaMemcpyToSymbol(SpheresCPU, SpheresGPU, sizeof(sphereStruct)*NUMSPHERES);
	//cudaErrorCheck(__FILE__, __LINE__);
	...
	printf("\nTime elapsed for on-GPU computations: %fms\n", elapsed_time);
...
void setup()
{
	...
	SpheresCPU = (sphereStruct*)malloc(sizeof(sphereStruct)*NUMSPHERES);
	cudaErrorCheck(__FILE__, __LINE__);
