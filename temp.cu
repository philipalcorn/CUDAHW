...
void cudaErrorCheck(const char *file, int line)
{
	...
	{
		printf("\n CUDA ERROR: meslsage = %s, File = %s, Line = %d\n", cudaGetErrorString(error), file, line);
		exit(0);
	}
}
...
__global__ void zero_forces(float3* F, int N)
{
    ...
    if (gid < N) F[gid] = make_float3(0.f, 0.f, 0.f);
}
...
__global__ void calculate_forces(const float3* pos, 
								...
								int N) 
{
	int lid = threadIdx.x; // We can just use the threadid since we only have one block
	int gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid >= N) return; // Only modify the first N bodies
    ...
    for (int j = 0; j < N; ++j) {
        if (j == gid) continue; // don't calculate force on the same bodies
        float3 pj = pos[j];
        float mj = mass[j];
                          ...
                          float dt, float damp, int N, int firstStep)
{
    ...
    float invMi = 1.0f / M[gid];
    float3 Vi = V[gid]; // get ith velocity
    float3 Fi = F[gid]; // get ith force
    ...
    V[gid] = Vi; // replace the old values with the new values
    P[gid] = Pi;
}
