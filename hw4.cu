...
// Defines
#define N 111111503 // Length of the vector
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	if(id < N) // Making sure we are not working on memory we do not own.
	{
        ...
        {
            // heavy computation (matches CPU)
            float aa = a[i];
            float bb = b[i];
            float t1 = sqrtf(cosf(aa)*cosf(aa) + aa*aa + sinf(aa)*sinf(aa) - 1.0f);
            float t2 = sqrtf(cosf(bb)*cosf(bb) + bb*bb + sinf(bb)*sinf(bb) - 1.0f);
            c[i] = t1 + t2;
        }
		//c[id] = a[id] + b[id];
	}
